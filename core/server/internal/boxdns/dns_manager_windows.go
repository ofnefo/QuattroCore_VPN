...
import (
	"QuattroCore/internal/boxdns/winipcfg"
	"encoding/binary"
...
func (d *DnsManager) HandleSystemDNS(ifc *control.Interface, flag int) {
	if d == nil {
		log.Println("No DnsManager, you may need to restart Quattro")
		return
	}
	if ifc != nil {
		if !dnsIsSet {
...
func (d *DnsManager) getInterfaceGuid(ifc control.Interface) (string, error) {
	if d.Monitor == nil {
		return "", E.New("No Dns Manager, you may need to restart Quattro")
	}
	index := ifc.Index
...
func (d *DnsManager) isIfcDNSDhcp(ifc control.Interface) (dhcp bool, err error) {
	if d == nil {
		log.Println("No DnsManager, you may need to restart Quattro")
		return false, E.New("No Dns Manager, you may need to restart Quattro")
	}
