...
if [[ "$GOOS" == "windows" || "$GOOS" == "linux" ]]; then
    UPDATER_EXT=$([[ "$GOOS" == "windows" ]] && echo ".exe" || true)
    $GOCMD build -v -o "$DEST/QuattroUpdater$UPDATER_EXT" -trimpath -ldflags "-w -s" ./cmd/quattro-updater
fi
...
CORE_EXT=$([[ "$GOOS" == "windows" ]] && echo ".exe" || true)
$GOCMD build -v -o "$DEST/QuattroCore$CORE_EXT" -trimpath -ldflags "-w -s -X 'github.com/sagernet/sing-box/constant.Version=${VERSION_SINGBOX}' -X 'internal/godebug.defaultGODEBUG=multipathtcp=0' -checklinkname=0" -tags "$TAGS" .
popd
